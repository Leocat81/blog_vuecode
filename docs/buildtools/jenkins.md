...
- 安装`Nodejs`
  ...
  > [nodejs 安装后执行打包异常](./jenkins.md#一些问题)
...
- 构建触发库
  ...
  2. 生成`Gitee webhook密码`并填写到自己的 gitee 项目设置中

- 构建环境

  1. `Provide Node & npm bin/ folder to PATH` 勾选系统配置中的`nodejs` (已在 jenkins 系统管理中配置成功)
...
- `nodejs`下载后无法使用
  ...
  1. 进入容器
  2. apk 配置镜像源 `sed -i 's/dl-cdn.alpinelinux.org/mirrors.aliyun.com/' /etc/apk/repositories`
  3. 下载 nodejs `apk add nodejs`
  4. 确认下载成功 `node -v`
  5. 去掉配置步骤中的软链
...
```bash
# 以root身份进入容器
$ docker exec -u root -it <容器ID或名称> bash
# 更新apt
$ apt update
# 下载zip
$ apt install zip
